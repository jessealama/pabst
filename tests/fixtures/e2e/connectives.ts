...
export function andB(a: boolean, b: boolean): boolean {
  return a && b;
}
export function notB(a: boolean): boolean {
  return !a;
}
...
export function isSmall(x: number): boolean {
  return x < 100;
}
export function inRange(x: number): boolean {
  return x > 0 && x < 100;
}
