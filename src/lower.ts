...
export function lowerExpr(f: Formula): string {
  switch (f.kind) {
    case "atom": return `__bool(${f.text}, ${JSON.stringify(f.text)})`;
    case "not": return `!(${lowerExpr(f.arg)})`;
    case "and": return `(${lowerExpr(f.left)} && ${lowerExpr(f.right)})`;
    case "or": return `(${lowerExpr(f.left)} || ${lowerExpr(f.right)})`;
    case "iff": return `(${lowerExpr(f.left)} === ${lowerExpr(f.right)})`;
    case "implication": return materialImplication(f.antecedents, f.consequent);
  }
}

function materialImplication(antecedents: Formula[], consequent: Formula): string {
 ...
 */
export function lowerTop(f: Formula): { preconditions: string[]; body: string } {
